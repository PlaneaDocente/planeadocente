  ...
  try {
    ...
    if (!productName || !amount || !currency) {
      return Response.json(
        { success: false, error: "Missing required fields: productName, amount, currency" },
        { status: 400 }
      );
    }
    ...
    if (!stripeSecretKey) {
      return Response.json(
        { success: false, error: "STRIPE_SECRET_KEY is not configured on the server. Add it to your Vercel environment variables." },
        { status: 500 }
      );
    }
    ...
    // Resolve plan_id from DB if not provided
    let resolvedPlanId: string | null = planId ?? null;
    if (!resolvedPlanId && productName) {
      const planName = productName.replace("PlaneaDocente ", "").split(" - ")[0];
      const { data: planRow } = await supabaseAdmin
        .from("subscription_plans")
        .select("id")
        .ilike("nombre", `%${planName}%`)
        .eq("activo", true)
        .maybeSingle();
